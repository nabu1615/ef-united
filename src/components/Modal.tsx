...
const Modal = ({ children, open, setOpen, title }: { children: React.ReactNode, open: boolean, setOpen: React.Dispatch<React.SetStateAction<boolean>>, title: string }) => {
    return (
        open ? <div id="default-modal" tabIndex={-1} aria-hidden="true" className="bg-white dark:bg-gray-900 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full flex">
